...
## Disclaimer!
This is only for the Windows operating system. Some of these would also work on Linux.
...
- Directory `include` for your header (`.h/.hpp`) files.
- Directory `src` for your implementation (`.c/.cpp`) files.
- Directory `build` for CMake build files.
- Create a `main.c`/`main.cpp` file in the `src` directory with your main function.
- File `CMakeLists.txt`
3. Copy this template into your `CMakeListsFile.txt` file:
```cmake
...
5. Run this to generate all the CMake files.
```bash
cmake ..
```
6. Run this to build your project.
```bash
cmake --build .
```
7. Run this to run your program.
```bash
./Debug/<executable file name>.exe
```
Replace `<executable file name>` with the name of your program.
